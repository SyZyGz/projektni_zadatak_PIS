...
uses
  ...
  FMX.Layouts, FMX.Edit, FrmPrincipal, databaseForm, SignUp, Unit14, GlobalVariables;
...
begin
  ...
  // Pripremi SQL upit za proveru korisnika i dobijanje privilegija
  database.FDQuery1.SQL.Text := 'SELECT ID, PRIVILAGE FROM users WHERE username = :username AND password = :password';
  ...
  try
    ...
    begin
      // Postavi globalne promenljive
      CurrentUserID := database.FDQuery1.FieldByName('ID').AsInteger;
      CurrentUserPrivilege := database.FDQuery1.FieldByName('PRIVILAGE').AsInteger;

      // Proveri privilegije korisnika
      if CurrentUserPrivilege = 1 then
      begin
        // Trener
