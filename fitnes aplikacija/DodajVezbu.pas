...
uses
  ...
  FMX.Objects, FMX.Layouts,GlobalVariables;
...
type
  TForm14 = class(TForm)
    ...
    Edit5: TEdit; // Labela za Ukljuceni Misici
    Button1: TButton;
    Pozadina: TLayout;
    S: TRectangle;
    Layout1: TLayout;
    ...
    Button2: TButton; // Dugme za unos u vezba
...
begin
  // Proveri privilegije korisnika pre brisanja
  if CurrentUserPrivilege = 1 then
  begin
    if not database.FDConnection1.Connected then
      database.FDConnection1.Connected := True;

    // Pripremi SQL upit za brisanje svih vežbi
    database.FDQuery1.SQL.Text := 'DELETE FROM vezba';
    try
      database.FDQuery1.ExecSQL;
      ShowMessage('All exercises have been deleted.');
    except
      on E: Exception do
        ShowMessage('Error: ' + E.Message);
    end;
  ...
  begin
    ShowMessage('You do not have permission to delete exercises.');
  end;
end;
